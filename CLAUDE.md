...
**Laravel and plain PHPStan.** `vendor/bin/phpstan` is Composer's bin proxy, which hands PHPStan Sentinel Slop's own `vendor/autoload.php` via `$GLOBALS['_composer_autoload_path']`. So when scanning a user repo, PHPStan can see Sentinel Slop's Laravel framework classes (whatever version we ship, not the user's) but has no Larastan extensions. That produces a specific set of misreadings on Eloquent code: `nullsafe.neverNull` on model properties (`$user?->name ?? ''` is defensive, not unnecessary), `return.type`/`return.phpDocType`/`argument.type`/`argument.unresolvableType` involving `Illuminate\` types (relations and scopes "return Query\Builder", `Dog::create()` "returns Model"). `PhpStanAnalyser::isLaravelMisreading()` drops those when composer declares `laravel/framework` (or Illuminate resolves in composer.lock) and keeps them for every other PHP codebase. On dog-kennel this removed 20 of 40 medium findings.
...
score    = max(0, round(base) - low - supp)
